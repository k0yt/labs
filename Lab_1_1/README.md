# Lab_1_1 Поиск аккаунтов сотрудников (OSINT)
...
Необходимо:
1. Скачать образ kali linux [lab.ova]() и запустить в консоли `echo "nameserver 127.0.0.1" | sudo tee /etc/resolv.conf`
2. Выполнить пассивный поиск ресурсов организации codovix.ru (поиск аккаунтов сотрудников методом OSINT)
3. Собрать полезную информацию о сотрудниках для следующего этапа пентеста (составить 2 парольных словаря - в которых будут все комбинации из 5 слов с сохранением регистра, без пробелов между словами)
